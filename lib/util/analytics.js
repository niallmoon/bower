var Insight = require('insight');
var mout = require('mout');
var config = require('../config');
var pkg = require('../../package.json');

var analytics = module.exports;
var insight;

// Initializes the application-wide insight singleton and asks for the
// permission on the CLI during the first run.
analytics.setup = function setup(cb) {
    insight = new Insight({
        trackingCode: 'UA-XXXXXXX-X',
        packageName: pkg.name,
        packageVersion: pkg.version
    });

    // Display the ask prompt only if it hasn't been answered before
    // and the current session is interactive.
    if (insight.optOut === undefined && config.interactive) {
        insight.askPermission(null, cb);
    } else {
        cb();
    }
};

analytics.track = function track() {
    if (!insight) {
        throw new Error('You must call analytics.setup() prior to tracking.');
    }
    insight.track.apply(insight, arguments);
};

analytics.trackDecomposedEndpoints = function trackDecomposedEndpoints(command, endpoints) {
    endpoints.forEach(function (endpoint) {
        analytics.track(command, [endpoint.source, endpoint.target].join('/'));
    });
};

analytics.trackPackages = function trackPackages(command, packages) {
    mout.object.forOwn(packages, function (package) {
        var meta = package.pkgMeta;
        analytics.track(command, [meta.name, meta.version].join('/'));
    });
};

analytics.trackNames = function trackNames(command, names) {
    names.forEach(function (name) {
        analytics.track(command, name);
    });
};
